...
void *pthreadStartRoutine(void *arg) {
    ...
    while (bee_PTR->beehive_PTR->num_nectar < bee_PTR->beehive_PTR->max_number_nectar) {
        ...
        if (work_flower) {
            if (food_source !=
                NULL) //if food_source is NULL -> something went wrong -> just ignore this cycle (hopefully works)
            {
            ...
            if (food_source != NULL && food_source->has_been_harvested == false) {
                ...
                if (food_source->width > 0) {
                    flower_t *nextfl = (bee_PTR->flowerfield_PTR +
                                        food_source->heigt * bee_PTR->flowerfield_PTR->ff_width +
                                        (food_source->width - 1));
                ...
                if (food_source->heigt > 0) {
                    flower_t *nextfl = (bee_PTR->flowerfield_PTR +
                                        (food_source->heigt - 1) * bee_PTR->flowerfield_PTR->ff_width +
                                        (food_source->width));
                    //if((bee_PTR->flowerfield_PTR + food_source->heigt * bee_PTR->flowerfield_PTR->ff_width + (food_source->width - 1))->has_been_harvested == false){
                    if (nextfl->has_been_harvested == false) {
                        pthread_error_funct(pthread_mutex_lock(bee_PTR->beehive_PTR->mutex_PTR));
                ...
                char comp[256] = {"\0"};
                if (strcmp(comp, left) == 0 && strcmp(comp, right) == 0 && strcmp(comp, up) == 0 &&
                    strcmp(comp, down) == 0) {
        ...
        if (ret ==
            0) { goto nextwait; }    //All but 1 thread: go to next pthread_barrier_wait so all threads are at the same location again.
...
int main(int argc, char *argv[]) {
    ...
    if (beehive.destroyed_by_bear == true) {
        printf("%llu bees collected nectar from %d/%llu flowers.\n", num_bees,
               beehive.actual_number_of_nectar_collected, (ff_height * ff_width));
